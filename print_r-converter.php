...
if (($iLength = strlen($i)) > ($iMaxLength = 4096)) {
...
if (!empty($i)) {
    ...
    setcookie('i', $i, (int)ini_get('session.cookie_lifetime'), ini_get('session.cookie_path'), ini_get('session.cookie_domain'), (bool)ini_get('session.cookie_secure'), (bool)ini_get('session.cookie_httponly'));
    $requestHasCookieData = true;
} elseif (!empty($_COOKIE['i'])) {
    $i = (string)$_COOKIE['i'];
    if (($iLength = strlen($i)) > $iMaxLength) {
...
<form action="" method="post" accept-charset="UTF-8">
    <?php if ($i):
        ...
        <div>
            <label>
                <textarea rows="16" wrap="soft" cols="80" style="overflow:auto; width:99%; white-space: pre;"
    ...
    <div>
        <textarea rows="16" wrap="soft" cols="80" style="overflow:auto; width:99%; white-space: pre;" id="i"
                  name="i"><?php echo htmlspecialchars($i, ENT_QUOTES, 'UTF-8'); ?></textarea>
    </div>
</form>
<a href="https://github.com/hakre/print_r-converter" target="_top"><img
